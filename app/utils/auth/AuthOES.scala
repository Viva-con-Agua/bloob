...
class OESConnection(authOESActor: ActorRef, conf : Configuration) {
  ...
  private val server = conf.getString("nats.endpoint").get
...
class AuthOESHandler @Inject() (conf : Configuration) extends Actor with Timers {
  ...
  val sessionTimeout : Long = conf.getMilliseconds("silhoutte.authenticator.authenticatorIdleTimeout").getOrElse(60000 * 10)
