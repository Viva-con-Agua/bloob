...
class MailerService @Inject() (mailerClient: MailerClient, mailDAO : MailDAO) {
  ...
  def send(mail: Mail) : Future[Option[Mail]] = {
    // every user has te receive her / his own e-mail; no mass mail with readable other addresses and no bcc
    for(receiver <- mail.receiver) {
      val email = Email(
        mail.subject,
        ...
        Seq(receiver), // TODO: If receiver are complete users, adopt schema: "Miss TO <to@email.com>"
